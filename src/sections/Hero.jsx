...
//   return (
//     <section className="bg-gradient-to-b from-white to-gray-0.3">
...
//           className="flex-1"
//         >
//           <h1 className="text-4xl md:text-4xl font-extrabold leading-tight tracking-tight text-gray-900">
...
//             <a
//               href="#contact"
//               className="btn-primary px-8 py-3 rounded-full font-semibold shadow-lg hover:shadow-xl transition-shadow duration-300 inline-flex items-center justify-center"
//             >
//               Get Started
//             </a>
//             <a
//               href="#services"
//               className="px-6 py-3 rounded-full border border-gray-300 text-gray-700 font-medium hover:border-primary hover:text-primary transition-colors duration-300"
//             >
//               Browse Services
//             </a>
//           </div>
...
// <div className="hidden sm:flex gap-6 bg-white p-8 rounded-2xl shadow-xl overflow-x-auto">
//   <h3 className="sr-only">Featured Assistants</h3> {/* visually hidden heading for accessibility */}
//   {assistants.map(({ name, role, img }, i) => (
//     <motion.div
//       key={i}
//       whileHover={{ y: -10, boxShadow: '0 10px 20px rgba(228,0,43,0.3)' }}
//       transition={{ type: 'spring', stiffness: 300, damping: 20 }}
//       className="flex flex-col items-center gap-3 p-4 border border-gray-200 rounded-lg cursor-pointer bg-white min-w-[160px]"
//     >
//       <img src={img} alt={name} className="w-16 h-16 rounded-full object-cover shadow-sm" />
...
// <div className="sm:hidden flex flex-col gap-6 bg-white p-6 rounded-2xl shadow-xl">
//   {assistants.map(({ name, role, img }, i) => (
//     <motion.div
//       key={i}
//       whileHover={{ y: -10, boxShadow: '0 10px 20px rgba(228,0,43,0.3)' }}
//       transition={{ type: 'spring', stiffness: 300, damping: 20 }}
//       className="flex items-center gap-4 p-4 border border-gray-200 rounded-lg cursor-pointer bg-white"
//     >
//       <img
//         src={img}
//         alt={name}
...
//     <section className="bg-[#e4002b] min-h-screen transform rotate-x-12  ">
...
//           className="flex-1"
//         >
//           <h1 className="text-4xl md:text-5xl font-extrabold leading-tight tracking-tight text-#e4002b drop-shadow-[0_0_10px_rgba(228,0,43,0.8)]">
//             Trusted Remote Assistants to Power Your Business
//           </h1>
//           <p className="mt-6 max-w-xl text-gray-300 text-lg leading-relaxed drop-shadow-[0_0_5px_rgba(255,255,255,0.2)]">
//             Hire skilled, vetted virtual assistants for admin, accounting, lead-gen, social media and more — scale quickly without the overhead.
//           </p>
...
// <h3 className="text-4xl font-extrabold text-[rgb(236,239,240)] mb-4 text-center">
//   Featured Assistants
// </h3>

// {/* Desktop & Tablet: horizontal row */}
// <div className="hidden sm:flex gap-6 bg-gray-900 bg-opacity-50 p-8 rounded-2xl shadow-[0_0_25px_rgba(228,0,43,0.7)] overflow-x-auto">
//   {assistants.map(({ name, role, img }, i) => (
//     <motion.div
//       key={i}
//       whileHover={{ y: -10, boxShadow: '0 10px 20px rgba(228,0,43,0.5)' }}
//       transition={{ type: 'spring', stiffness: 300, damping: 20 }}
//       className="flex flex-col items-center gap-3 p-4 border border-red-600 rounded-lg cursor-pointer bg-gray-800 min-w-[160px]"
//     >
//       <img
//         src={img}
//         alt={name}
//         className="w-16 h-16 rounded-full object-cover shadow-lg shadow-red-600/60"
//       />
//       <h4 className="font-semibold text-lg text-red-400 drop-shadow-[0_0_5px_rgba(228,0,43,0.8)]">{name}</h4>
...
//           {/* Mobile: vertical stacked cards */}
//           <div className="sm:hidden flex flex-col gap-6 bg-gray-900 bg-opacity-50 p-6 rounded-2xl shadow-[0_0_25px_rgba(228,0,43,0.7)]">
//             {assistants.map(({ name, role, img }, i) => (
//               <motion.div
//                 key={i}
//                 whileHover={{ y: -10, boxShadow: '0 10px 20px rgba(228,0,43,0.5)' }}
//                 transition={{ type: 'spring', stiffness: 300, damping: 20 }}
//                 className="flex items-center gap-4 p-4 border border-red-600 rounded-lg cursor-pointer bg-gray-800"
//               >
//                 <img
//                   src={img}
//                   alt={name}
        ...
        <div className="flex-1 w-full relative">
          ...
          <div className="hidden sm:flex gap-6 bg-gray-900 bg-opacity-50 p-8 rounded-2xl shadow-[0_0_25px_rgba(228,0,43,0.7)] overflow-x-auto">
            {assistants.map(({ name, role }, i) => (
              <motion.div
                key={i}
                whileHover={{ scale: 1.05, y: -6, boxShadow: '0 10px 20px rgba(228,0,43,0.6)' }}
                transition={{ type: 'spring', stiffness: 300, damping: 20 }}
                className="relative flex flex-col items-center gap-3 p-4 border border-red-600 rounded-lg bg-gray-800 min-w-[160px] shadow-lg hover:shadow-[0_0_20px_rgba(228,0,43,0.8)] group"
              >
                <div className="w-16 h-16 rounded-full overflow-hidden shadow-lg shadow-red-600/60 bg-gray-700 flex items-center justify-center">
                  <img
                ...
                <h4 className="font-semibold text-lg text-red-400">{name}</h4>
                <p className="text-sm text-red-500 font-medium">{role}</p>
                <span className="absolute left-0 bottom-0 w-full h-[3px] bg-red-400 opacity-70 animate-pulse group-hover:w-0 transition-all duration-500"></span>
              </motion.div>
            ))}
          </div>
          ...
          <div className="sm:hidden flex flex-col gap-6 bg-gray-900 bg-opacity-50 p-6 rounded-2xl shadow-[0_0_25px_rgba(228,0,43,0.7)]">
            {assistants.map(({ name, role }, i) => (
              <motion.div
                ...
                className="relative flex items-center gap-4 p-4 border border-red-600 rounded-lg bg-gray-800 shadow-lg hover:shadow-[0_0_20px_rgba(228,0,43,0.8)] group"
              >
                <div className="w-16 h-16 rounded-full overflow-hidden shadow-lg shadow-red-600/60 bg-gray-700 flex items-center justify-center">
                  <img
                    src="https://via.placeholder.com/64"
